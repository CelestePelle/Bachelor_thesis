...
world_mortality_df <- world_mortality#read.csv("World_Mortality/world_mortality.csv", sep = ",")
...
#one-hot encoding for weeks or months
  ...
  predictors_df$Response <- data[pre, 6] #number of deaths per time unit

#fit gls with AR(1)
  model <- nlme::gls(Response ~ 0 + ., data = predictors_df, correlation=corAR1(form= ~1))
  summary(model)
  intervals(model)
...
# ################################################################################  
  ...
  colnames(predictors2020) <- c('year', paste0('X', 1:m))
#predict baseline values for 2020 using the regression model
  baseline <- stats::predict(model, newdata = as.data.frame(predictors2020))
...
#calculating excess mortality as defined in article
  print(diff2021)
  total_diff <- sum(diff2020[excess_begin:length(diff2020)])+ sum(diff2021[1:24]) 
