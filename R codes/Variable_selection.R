#Variable selection using ANOVA and AIC
...
removed <- lapply(seq_along(countries_sorted), function(x) rep(0, 12)) #list to store what covariates are removed per country
...
for(country in countries_sorted){
  ...
  selection <- (data$Year %in% c(2015,2016,2017,2018)) & (data$Unit<53) #test 2019
  ...
  predictors_complete <- predictors_full[,c(1,3:(m+13))] #all predictors used in model 4
  smallest_diff <- -10
  covariates <- c(1,2,m+3,m+4,m+5,m+6,m+7,m+8,m+9,m+10,m+11,m+12) #index of the variable columns in data set
  while(smallest_diff < 1){ #while improving
    p_values <- rep(0,length(covariates))
    AIC_diff <- rep(0, length(covariates))
    l = 0
    #fit complete model
    model_complete <- nlme::gls(Deaths ~ 0 + ., data=predictors_complete, method = "ML", correlation = corAR1(form = ~1)) 
    for(i in covariates){
      l = l + 1
      
      #fit model with one variable left out
      if(i==2){ # exclude weeks/months
        model_subset <- nlme::gls(Deaths ~ 0 + ., data=predictors_complete[,-c(2:(m+1))],method = "ML", correlation = corAR1(form = ~1))
      }else{
        model_subset <- nlme::gls(Deaths ~ 0 + ., data=predictors_complete[,-i], method = "ML", correlation = corAR1(form = ~1))
      }
      comparison <- anova(model_subset, model_complete)
      p_values[l] <- comparison$"p-value"[2]
      AIC_diff[l] <- AIC(model_subset) - AIC(model_complete)
    }#for covariates
    smallest_diff <- min(AIC_diff)
    red <- which.min(AIC_diff)
    if(p_values[red] > 0.1 || (p_values[red] > 0.05 && smallest_diff < -4)){
      if(covariates[red] >2){
        ind <- covariates[red] - m
      }else{
        ind <- covariates[red]
      }
      covariates <- covariates[-red]
      removed[[c]][ind] <- 1
    }else{
      break
    }
